...
export class MockAIProvider implements AIProvider {
  async analyzeGoal(input: GoalInput): Promise<GoalAnalysisResult> {
    ...
    } else if (lowerTitle.includes('blender') || lowerTitle.includes('3d') || lowerTitle.includes('modelleme') || lowerTitle.includes('animasyon') || lowerTitle.includes('render')) {
      tasks = [
        {
          id: 'task-1',
          dayNumber: 1,
          title: 'Blender 3D Arayüzü & Obje Manipülasyonu',
          description: 'Blender 3D görünüm penceresini incele, küp ekle/sil ve G (Taşı), R (Döndür), S (Ölçekle) kısayollarını pratik et.',
          taskType: 'learning_and_practice',
          durationMinutes: input.dailyMinutes,
          difficulty: 1,
          successCriteria: 'Nesneleri 3 eksende (X, Y, Z) serbestçe hareket ettirebilmek',
          freeResourceUrl: 'https://docs.blender.org',
          status: 'pending'
        },
        {
          id: 'task-2',
          dayNumber: 2,
          title: 'Edit Mode, Extrude (E) & Loop Cut (Ctrl+R)',
          description: 'Tab tuşu ile Edit Mode’a geç. Vertex, Edge ve Face seçimi yapıp Extrude ve Loop Cut ile nesneyi şekillendir.',
          taskType: 'learning_and_practice',
          durationMinutes: input.dailyMinutes,
          difficulty: 2,
          successCriteria: 'Temel bir 3D sandalye veya masa modeli oluşturmak',
          status: 'pending'
        },
        {
          id: 'task-3',
          dayNumber: 3,
          title: 'Subdivision Surface & Mirror Modifier',
          description: 'Modifier paneline gir. Subdivision Surface ile nesneyi pürüzsüzleştir, Mirror ile simetrik modelle.',
          taskType: 'learning_and_practice',
          durationMinutes: input.dailyMinutes,
          difficulty: 2,
          successCriteria: 'Simetrik pürüzsüz bir 3D obje modellemek',
          status: 'pending'
        },
        {
          id: 'task-4',
          dayNumber: 4,
          title: 'Material & Shader Editor ile Renklendirme',
          description: 'Shading sekmesine geç. Base Color, Roughness ve Metallic değerlerini ayarlayarak malzemeye renk ve parlaklık ver.',
          taskType: 'learning_and_practice',
          durationMinutes: input.dailyMinutes,
          difficulty: 2,
          successCriteria: 'Modelin üzerine cam, metal ve plastik materyalleri uygulamak',
          status: 'pending'
        },
        {
          id: 'task-5',
          dayNumber: 5,
          title: 'Işıklandırma (Point/Sun) & Kamera Yerleşimi',
          description: 'Sahneye 3 noktalı ışıklandırma kur, Numpad 0 ile kamera açısını ayarla.',
          taskType: 'learning_and_practice',
          durationMinutes: input.dailyMinutes,
          difficulty: 2,
          successCriteria: 'Kamera kadrajını ve estetik sahne ışığını tamamlamak',
          status: 'pending'
        },
        {
          id: 'task-6',
          dayNumber: 6,
          title: 'Eevee ve Cycles Render İhracı',
          description: 'Render motorunu seç (Eevee veya Cycles), çözünürlüğü ayarla ve ilk PNG görsel çıktını (F12) al.',
          taskType: 'review',
          durationMinutes: input.dailyMinutes,
          difficulty: 2,
          successCriteria: 'Yüksek kaliteli 3D görsel render çıktısı kaydetmek',
          status: 'pending'
        },
        {
          id: 'task-7',
          dayNumber: 7,
          title: 'İlk 3D Model Portföy Çıktısı & İnceleme',
          description: 'Hafta boyunca yaptığın 3D çalışmayı incele, hataları düzelt ve portföy kütüphanene ekle.',
          taskType: 'review',
          durationMinutes: input.dailyMinutes,
          difficulty: 3,
          successCriteria: 'Tamamlanmış 3D model görselini kaydetmek',
          status: 'pending'
        }
      ];
    } else if (input.category === 'exam_study' || lowerTitle.includes('yks') || lowerTitle.includes('tyt') || lowerTitle.includes('sınav')) {
      tasks = [
        {
          id: 'task-1',
          dayNumber: 1,
